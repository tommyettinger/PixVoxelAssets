...
namespace AssetsPV
{
    ...
    public class PatternLogic
    {
        ...
        /// <summary>
        /// http://and-what-happened.blogspot.nl/2011/08/fast-2d-and-3d-hilbert-curves-and.html
        /// </summary>
        /// <param name="morton">Generated by MortonEncode</param>
        /// <param name="bits">The number of bits used for a side length</param>
        /// <returns>A Hilbert Index</returns>
        public static int MortonToHilbert2D(int morton, short bits)
        {
            int hilbert = 0;
            int remap = 0xb4;
            int block = (bits << 1);
            while(block != 0)
            {
                ...
                remap ^= (int)(0x82000028L >> (hcode << 3));
                hilbert = ((hilbert << 2) + hcode);
            }
            return (hilbert);
        }
        public static int HilbertCode2D(int x, int y)
        {
            return MortonToHilbert2D(MortonEncode((byte)x, (byte)y), 8);
        }
